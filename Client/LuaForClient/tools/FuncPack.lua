...
function FuncPack:keepDigital(num)
	return string.format("%0.2f", num);
end

function FuncPack:getTimeForWalk(pos1, pos2)
	local point1 = FuncPack:PositionTopoint(pos1);
	local point2 = FuncPack:PositionTopoint(pos2);
    local distance = FuncPack:getAbsoluteDistanceWithOPoints(point1, point2);

	local scaleDistance = distance/TILE_WIDTH;
...
function FuncPack:getDistanceWithOPoints(point1, point2)

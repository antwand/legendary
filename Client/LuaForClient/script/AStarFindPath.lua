...
function AStarFindPath:getAStarPath(to, from, _map, maxStep)
    ...
    while list ~= nil do
        table.insert(path, #path+1, {x=list.x*TILE_WIDTH, y=list.y*TILE_HEIGHT});
        list = list.parent;
    end

    table.insert(path, 1, {x=to.x*_map.tileSize.width, y=to.y*_map.tileSize.height});
    path[#path] = nil;
...
function AStarFindPath:aStarFindPath(to, from, _map)
    ...
    while self:arrive(open_list[1], to) == false do
        ...
        --删除已经检测的节点
        self:signValidOfPoint(open_list[1], false);

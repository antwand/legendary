...
const char* ccRemind_fsh = STRINGIFY(
	...
	void main()
	{
        ...
        gl_FragColor = pcolor;
		//float gray = dot(pcolor.rgb, vec3(0.0, 0.0, 1.0));
		//gl_FragColor = vec4(vec3(gray), pcolor.a);
	}
	);
...
bool RemindShader::init()
{
	...
	m_ptLastScenePos.x = -1;
	m_ptLastScenePos.y = -1;
...
void RemindShader::Render()
{
	...
	Vec2 parentDelta;
	Vec2 parentPos = this->getParent()->getPosition();
	if (m_ptLastScenePos.x == -1 && m_ptLastScenePos.y == -1)
		parentDelta = Vec2(0, 0);
	else if(parentPos != m_ptLastScenePos)
		parentDelta = (parentPos - m_ptLastScenePos)*ACTION_DISTANCE_SCALE;

	m_ptLastScenePos = parentPos;
	...
	{    
      ...
      //  Point delta = _position;// this->convertToWorldSpace(_position);
		GLfloat vertices[] = {
			x   - delta.x + parentDelta.x,y+ h - delta.y + parentDelta.y,
			x+w - delta.x + parentDelta.x,y+ h - delta.y + parentDelta.y,
			x - delta.x + parentDelta.x, y - delta.y + parentDelta.y,
			x+w - delta.x + parentDelta.x, y - delta.y + parentDelta.y
		};

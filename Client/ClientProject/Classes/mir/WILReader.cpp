...
WILReader* WILReader::m_pInstance = 0;

WILReader::WILReader(void)
{
	palletes[0][0] = 0;
	palletes[0][1] = 0;
	palletes[0][2] = 0;
...
WILReader* WILReader::getInstance()
{
	if (m_pInstance == 0)
		m_pInstance = new WILReader();

	return m_pInstance;
}

vector<MirImageInfo*> WILReader::readMirImageInfos(vector<int> idxs,
...
MirImageInfo*  WILReader::readMirImageInfo(int index,
			RandomAccessFile* imageFile,
			RandomAccessFile* confFile)
{
	byte bytesInt[4]; 
	...
	imageFile->seek(0);
	imageFile->skipBytes(48);
	imageFile->read(bytesInt);

	colorCount = Common::ColorCountToBitCount(Common::ReadInt(bytesInt, 0, true));
	if (colorCount < 16) {
		// 8位灰度图可能版本标识不为0，此时操作不一样
		imageFile->skipBytes(4); // 调色板使用默认
		imageFile->read(bytesInt);
		verFlag = Common::ReadInt(bytesInt, 0, true); // 版本标识
	}
	...
	if (verFlag != 0) {
		confFile->skipBytes(4); // 版本标识不对需要多跳过标识的4字节
	}
	...
	int length = 0;
	if (position + 4 < confFile->getLength())
	{
		confFile->seek(position + 4);
		confFile->read(bytesInt);
		int nextOffset = Common::ReadInt(bytesInt, 0, true);
		length = nextOffset - a_offset;
	}
	else
		length = confFile->getLength() - a_offset;
	...
	if (length <= 0 || a_offset <= 0 || mirImageInfo->getWidth() <= 0 || mirImageInfo->getWidth()>= 10000)
	{
		mirImageInfo->setOffsetX(0);
		mirImageInfo->setOffsetY(0);
		mirImageInfo->setDataSize(2);
...
MirImageInfo* WILReader::read(int position, RandomAccessFile* imageFile)
{
	...
	res->setWidth(Common::ReadShort(imageFile->getFileData(), position + 0, true));
	res->setHeight(Common::ReadShort(imageFile->getFileData(), position + 2, true));
	res->setOffsetX(Common::ReadShort(imageFile->getFileData(), position + 4, true));
	res->setOffsetY(Common::ReadShort(imageFile->getFileData(), position + 6, true));

	return res;
}

void WILReader::readPixels(MirImageInfo* mirImageInfo,
			RandomAccessFile* imageFile)
{
	...
	imageFile->seek(offset + 8);
	imageFile->read(pixels, length);
	...
	if (colorCount == 8) {
		int p_index = 0;
		for (int h = mirImageInfo->getHeight() - 1; h >= 0; --h)
			for (int w = 0; w < mirImageInfo->getWidth(); ++w) {
				// 跳过填充字节
				if (w == 0)
		...
		for (int h = height - 1; h >= 0; --h)
		{
			for (int w = 0; w < width; ++w, p_index += 2) {
				...
				if (data == 0)
				{
					...
					unpackedPixels[(h * width + w) * 4] = br;
					unpackedPixels[(h * width + w) * 4 + 1] = bg;
					unpackedPixels[(h * width + w) * 4 + 2] = bb;
					unpackedPixels[(h * width + w) * 4 + 3] = (byte)255;
				}
			}
		}
	}
...
bool WILReader::checkBlackColor(int R, int G, int B)
{
	if (R == 8 && G == 24 && B == 16)
		return true;
...
void WILReader::setClearNearBlackColor(bool clear)
{

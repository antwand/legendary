...
int SimulatorWin::run()
{
	...
	std::string winTitle = ConfigParser::getInstance()->getInitViewName();
    auto glview = GLViewImpl::createWithRect(winTitle.c_str(), frameRect, frameScale);
    ...
    RECT rect;
    GetWindowRect(_hwnd, &rect);
    MoveWindow(_hwnd, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, FALSE);
